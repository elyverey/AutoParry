...
local STATE={
    ...
    parryVisualCollapseTime=0.08,
    parryVisualExpandTime=0.14,
...
do
    ...
    local function isAttackDirectionValid(killerChar,profile)
        local localChar=localPlayer.Character
        local localRoot=localChar and localChar:FindFirstChild("HumanoidRootPart")
        local killerRoot=killerChar and killerChar:FindFirstChild("HumanoidRootPart")
        ...
        local allowedAngle=45+(90*aimPercent)

        if profile and profile.type=="lungehold" then
            allowedAngle=math.min(90,allowedAngle+8)
        end

        local dot=flatLook.Unit:Dot(flatDirection.Unit)
        local threshold=math.cos(math.rad(math.min(90,allowedAngle)))
        ...
        if flatVelocity.Magnitude>5 then
            local velocityDot=flatVelocity.Unit:Dot(flatDirection.Unit)
            local velocityThreshold=math.cos(math.rad(math.min(90,allowedAngle+8)))
    ...
    local function canNormalAttackParry(killerChar,distance,closingSpeed,predictedDistance,profile)
        ...
        if isAttackDirectionValid(killerChar,profile) then
            return true
        end
    ...
    local function hiddenCanParry(char,seq)
        ...
        if not seq.armed and not seq.skillLongEnough then
            return false
        end

        if not seq.skillFinished and not seq.skillLongEnough then
            return false
        end
        ...
        local predictedDistance=getPredictedDistance(char,0.50)
    ...
    function toggleAutoParryESP(state)
        if state then
            ...
            espLoop=RunService.RenderStepped:Connect(function()
                ...
                local pulseElapsed=tick()-pulseAt

                if pulseAt>0 and pulseElapsed<STATE.parryVisualCollapseTime then
                    local t=math.clamp(
                        pulseElapsed/STATE.parryVisualCollapseTime,
                        0,
                        1
                    )
                    radius=STATE.autoParryRadius*(1-t)
                elseif pulseAt>0 and pulseElapsed<STATE.parryVisualCollapseTime+STATE.parryVisualExpandTime then
                    local t=math.clamp(
                        (pulseElapsed-STATE.parryVisualCollapseTime)/STATE.parryVisualExpandTime,
                        0,
                        1
                    )
                ...
                if math.abs(radius-lastRadius)>0.02 then
