...
local KillerAimingFeature=(function()
    ...
    local function createLine(store,char,color,prefix)
        ...
        -- Anchor the beam reference to the HumanoidRootPart (torso/body),
        -- while the endpoint is projected from the center of the Head.
        local root=char:FindFirstChild("HumanoidRootPart")
        startAttachment.Parent=root or head
    ...
    local function updateLine(data,char)
        ...
        if data.beam and (data.beam.Name=="LineOfSightAlways" or data.beam.Name=="LineOfSightAlwaysYellow") then
            length=math.clamp(alwaysLineLength,10,100)
        end
    ...
    local function setAlwaysLineLength(v)
        alwaysLineLength=math.clamp(tonumber(v) or 115,10,100)
        if lineAlways then
            local killer=getCurrentKiller()
            if killer then
                if not alwaysLines[killer] then
