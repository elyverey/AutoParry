...
llocal KillerAimingFeature=(function()
    local activeTracks={}
    ...
    local function updateLine(data,char)
        ...
        local length=math.clamp(tonumber(STATE.alwaysLineLength) or 50,10,100)
    ...
    local LINE_OF_SIGHT_LENGTH=100

    local function getActiveLineOfSightState(char)
        local humanoid=char and char:FindFirstChildOfClass("Humanoid")
        local animator=humanoid and humanoid:FindFirstChildOfClass("Animator")
        if not animator then
            return nil
        ...
        -- Read the tracks that are actually playing right now.
        -- Do not rely only on AnimationPlayed/Stopped callbacks because
        -- tracks can be started/stopped between RenderStepped frames.
        for _,track in ipairs(animator:GetPlayingAnimationTracks()) do
            if track and track.IsPlaying then
                local id=numericId(track)
                local mode=id and KILLER_AIMING_ANIMS[id]
    ...
    local function updateAimingLine(char,color)
        local data=aimingLines[char]
        if not data then
            return
        end

        updateLine(data,char)
        -- Line of Sight has its own fixed length: 100 studs.
        local head=char and char:FindFirstChild("Head")
        local root=char and char:FindFirstChild("HumanoidRootPart")
        if head and root then
            local direction=root.CFrame.LookVector
            if direction.Magnitude>.001 then
                data.endpointPart.CFrame=CFrame.new(
                    head.Position + direction.Unit*LINE_OF_SIGHT_LENGTH
                )
            end
        end
    ...
    local function hasActiveYellowAnimation(char)
        ...
        for track,data in pairs(tracks) do
            ...
            elseif data and data.yellow then
                yellowActive=true
            end
        end
    ...
    local function updateAlwaysLine(char)
        ...
        -- Always Line keeps a white base, but switches the actual Beam
        -- to yellow while any of the configured lunge/corrupt animations
        -- is actively playing. When the animation ends it returns to white.
        local yellowActive=hasActiveYellowAnimation(char)
        local color=yellowActive and YELLOW or WHITE

        if data.color~=color then
            data.color=color
            setBeamColor(data.beam,color)
        else
            -- Keep the Beam color synchronized even if another animation
            -- track was registered/stopped between RenderStepped frames.
            setBeamColor(data.beam,color)
        end
        data.beam.Enabled=true
    end
    ...
    local function heavenGroundPoint(char,point)
        ...
        -- Keep the whole Heaven Eye on the ground plane used by the current
        -- working implementation, without letting it sink into the floor.
    ...
    local function ensureHeavenParts(char)
        ...
        -- 2 side segments + 7 short arc segments.
        -- Together they form the rounded/fan shape shown in the reference image.
        list={}
        for i=1,9 do
    ...
    local function updateHeavenEyes(char)
        ...
        -- Bottom point of the Heaven Eye.
        local apex=Vector3.new(
            root.Position.X + forward.X*1.0,
            groundY,
            root.Position.Z + forward.Z*1.0
        )

        -- Total width = 15.7 studs (7.85 each side).
        local halfWidth=7.85
        local depth=HEAVEN_DISTANCE
        local arcBulge=2.25

        local leftBase=Vector3.new(
            root.Position.X + forward.X*depth - right.X*halfWidth,
            groundY,
            root.Position.Z + forward.Z*depth - right.Z*halfWidth
        )

        local rightBase=Vector3.new(
            root.Position.X + forward.X*depth + right.X*halfWidth,
            groundY,
            root.Position.Z + forward.Z*depth + right.Z*halfWidth
        )
        ...
        -- Two straight lower sides.
        pointPart(parts[1],apex,leftBase)
        pointPart(parts[2],apex,rightBase)

        -- Rounded upper arc split into 7 short segments.
        local arcPoints={}
        local arcCenter=root.Position + forward*depth

        for i=0,7 do
            local t=math.pi - (math.pi*i/7)
            local lateral=math.cos(t)*halfWidth
            local forwardOffset=math.sin(t)*arcBulge

            local point=arcCenter
                + right*lateral
                + forward*forwardOffset

            arcPoints[i+1]=Vector3.new(point.X,groundY,point.Z)
        end
    ...
    local function setAlwaysLineLength(v)
        ...
        -- Always Line length is configurable. Line of Sight is intentionally
        -- fixed at 100 studs and does not inherit the Always Line slider.
        for char,data in pairs(aimingLines) do
            if data then data.length=LINE_OF_SIGHT_LENGTH end
            if char and char.Parent then
                local color=getActiveLineOfSightState(char)
                if color then
                    updateAimingLine(char,color)
                end
            end
        end
    end
    ...
    RunService.RenderStepped:Connect(function()
        ...
        if STATE.killerAimingEnabled then
                ...
                if player~=localPlayer and player.Character then
                    ...
                    if not isKillerCharacter(char) then
                        ...
                        if color then
                            local data=aimingLines[char]
                            if not data then
                                data=createAimingLine(char,color)
                            end
            ...
            for char in pairs(aimingLines) do
                if not char or not char.Parent
                or not isKillerCharacter(char)
                or not getActiveLineOfSightState(char) then
                    clearAimingLine(char)
                end
            end
        end
