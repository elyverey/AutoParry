...
do
    ...
    local function canNormalAttackParry(killerChar,distance,closingSpeed,predictedDistance)
        ...
        if closingSpeed and predictedDistance then
            if closingSpeed>=2.5
            and predictedDistance<=STATE.autoParryRadius+1.25 then
                return true
            end
        end
    ...
    local function hiddenTryWalkParry(char,seq)
        ...
        if seq.skillTrack and not seq.skillTrack.IsPlaying then
            return false
        end
    ...
    local function OnParryCharacterAdded(player,char)
        ...
        animator.AnimationPlayed:Connect(function(track)
            ...
            if currentName=="Unknown Killer" then
                local known=STATE.killerCharacters[char]
            ...
            espLoop=RunService.RenderStepped:Connect(function()
                ...
                if pulseAt>0 then
                    ...
                    if elapsed<STATE.parryVisualCollapseTime then
                        ...
                        local cooldownElapsed=elapsed
                        local expandStart=PARRY_COOLDOWN-STATE.parryVisualExpandTime

                        if cooldownElapsed<expandStart then
                            radius=0
                        else
                            if expandAt<=0 then
                                STATE.parryVisualExpandAt=pulseAt+expandStart
                                expandAt=STATE.parryVisualExpandAt
                            end

                            local expandElapsed=math.max(
                                now-expandAt,
                                0
                            )
                            local t=math.clamp(
