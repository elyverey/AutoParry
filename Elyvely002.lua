...
do
    ...
    local function canNormalAttackParry(killerChar,distance,closingSpeed,predictedDistance)
        ...
        -- A dropped pallet is NOT treated as a wall/parry source. If it fully
        -- blocks the route, don't parry from the opposite side.
        if obstacle.palletDropped and obstacle.blocked and not obstacle.edgeThreat then
            return false
        end

        -- If any ray reaches the killer while another ray is blocked, allow
        -- the parry: this catches attacks that can connect around object edges.
        if obstacle.edgeThreat then
            return true
        end
...
local KillerAimingFeature=(function()
    ...
    local function registerTrack(char,track)
        ...
        -- Animation-only killer lines are intentionally suppressed.
        -- The separate "Line Always" feature remains available.
        return
    end
