...
local KillerAimingFeature=(function()
    ...
    local function updateAimingLine(char)
        ...
        if color then
            ...
            data.beam.Width0=(color==YELLOW and 0.12 or LINE_THICKNESS)
            data.beam.Width1=(color==YELLOW and 0.12 or LINE_THICKNESS)
    ...
    local function updateAlwaysLine(char)
        ...
        if data.color~=color then
            ...
            -- Keep the Beam color synchronized every frame.
            setBeamColor(data.beam,color)
        end
        data.beam.Width0=(color==YELLOW and 0.12 or LINE_THICKNESS)
        data.beam.Width1=(color==YELLOW and 0.12 or LINE_THICKNESS)
    ...
    local function updateHeavenEyes(char)
        ...
        -- Exact reference shape: a centered bottom point with two straight
        -- lower edges and a rounded upper arc. Total width = 15.7 studs.
        ...
        -- Seven-segment rounded upper arc. Endpoints exactly meet the
        -- lower sides so there is no gap at either corner.
        local arcPoints={}
        for i=0,7 do
            local t=i/7
    ...
    local function getLiveAnimationColor(char)
        local humanoid=char and char:FindFirstChildOfClass("Humanoid")
        local animator=humanoid and humanoid:FindFirstChildOfClass("Animator")
        if not animator then
            return nil
    ...
    local function registerTrack(char,track)
        ...
        if not line then
            ...
            line.beam.Width0=(color==YELLOW and 0.12 or LINE_THICKNESS)
            line.beam.Width1=(color==YELLOW and 0.12 or LINE_THICKNESS)
        end
