...
local KillerAimingFeature=(function()
    ...
    local function raycastGroundPoint(char,point)
        local params=RaycastParams.new()
        params.FilterType=Enum.RaycastFilterType.Exclude
        params.FilterDescendantsInstances={char}
        params.IgnoreWater=true
        local result=Workspace:Raycast(
            point+Vector3.new(0,50,0),
            Vector3.new(0,-200,0),
            params
        )
        if result then
            return result.Position+Vector3.new(0,.04,0)
        end
        return point+Vector3.new(0,HEAVEN_HEIGHT,0)
    end
    ...
    local function updateHeavenEyes(char)
        ...
        local apex=raycastGroundPoint(char,apexBase)
        local leftEnd=raycastGroundPoint(char,leftBase)
        local rightEnd=raycastGroundPoint(char,rightBase)
