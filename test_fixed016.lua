...
do
    ...
    local function parryInputAvailable(now)
        if now-(STATE.lastParryTime or 0)<0.01 then
            return false
        end

        if now-(STATE.lastSkillParryTime or 0)<0.01 then
            return false
        end
    ...
    local function isFacingLocalPlayer(killerChar)
        ...
        local faceThreshold=math.clamp(threshold,-1,1)
        ...
        if flatVelocity.Magnitude>8 then
            ...
            if velocityDot>=math.max(threshold-0.10,0.30) then
                return true
            end
        end
    ...
    local function isAttackDirectionValid(killerChar,profile,closingSpeed)
        local localChar=localPlayer.Character
        local localRoot=localChar and localChar:FindFirstChild("HumanoidRootPart")
        local killerRoot=killerChar and killerChar:FindFirstChild("HumanoidRootPart")
        ...
        local targetDir=flatDirection.Unit
        local aimStrictness=math.clamp(STATE.aimStrictness,0.1,3.0)

        -- 0.1 = strictest, 3.0 = loosest.
        local toleranceDegrees=8+((aimStrictness-0.1)/2.9)*52
        local threshold=math.cos(math.rad(toleranceDegrees))

        local look=killerRoot.CFrame.LookVector
        local flatLook=Vector3.new(look.X,0,look.Z)

        if flatLook.Magnitude>0.05 then
            local lookDot=flatLook.Unit:Dot(targetDir)

            if lookDot>=threshold then
                return true
            end
        end

        -- For lunges, movement direction is also treated as attack direction.
        if profile and profile.type=="lungehold" and closingSpeed and closingSpeed>=3 then
            local velocity=killerRoot.AssemblyLinearVelocity
            local flatVelocity=Vector3.new(velocity.X,0,velocity.Z)

            if flatVelocity.Magnitude>3 then
                local velocityDot=flatVelocity.Unit:Dot(targetDir)

                if velocityDot>=threshold then
                    return true
                end
            end
        end
    ...
    local function canNormalAttackParry(killerChar,distance,closingSpeed,predictedDistance,profile)
        if distance>STATE.autoParryRadius then
            return false
        end

        if not isFacingLocalPlayer(killerChar) then
            return false
        end

        if not isAttackDirectionValid(killerChar,profile,closingSpeed) then
            return false
        end
...
local KillerAimingFeature=(function()
    ...
    local DASH_LENGTH=2.0
    local GAP_LENGTH=1.25
    local LINE_THICKNESS=0.055
    local BUILD_TIME=0.12

    local HEAVEN_DISTANCE=20
    local HEAVEN_SEGMENTS=8
    local HEAVEN_CURVE=0.55
    ...
    local function isKillerCharacter(char)
        ...
        if data and data.name and data.name~="Unknown Killer" then
            return true
        end
        ...
        for _,player in ipairs(Players:GetPlayers()) do
            if player~=localPlayer and player.Character==char then
                local values=char:FindFirstChild("Values")
                local killerName=values and values:FindFirstChild("KillerName")

                if killerName
                and killerName:IsA("StringValue")
                and killerName.Value~="" then
                    return true
                end
                ...
                if team and team.Name:lower():find("killer",1,true) then
                    return true
                end
            end
        end
    ...
    local function clearAimingParts(char)
        local list=aimingParts[char]
    ...
    local function raycastSegment(char,a,b)
        local delta=b-a
    ...
    local function raycastForward(char,origin,direction,distance)
        local params=RaycastParams.new()
        params.FilterType=Enum.RaycastFilterType.Exclude
        params.FilterDescendantsInstances={char}
        params.IgnoreWater=true
    ...
    local function ensureAimingParts(char,count)
        local list=aimingParts[char] or {}

        while #list<count do
            list[#list+1]=makePart(
                "KillerAimingLine",
                WHITE
            )
        end

        aimingParts[char]=list
        return list
    end

    local function getCurrentAiming(char)
        ...
        for track,data in pairs(tracks) do
            ...
            and track.IsPlaying then
                if not selected then
                    selected=data
                elseif data.yellow and not selected.yellow then
                    selected=data
                elseif data.yellow==selected.yellow
                and data.startedAt>selected.startedAt then
                    selected=data
                end
            end
        end
    ...
    local function updateAiming(char,data)
        if not STATE.killerAimingEnabled
        or not char
        or not char.Parent then
            clearAimingParts(char)
            return
        end
        ...
        local fullDistance=(hitPoint-origin).Magnitude

        local buildProgress=1

        if not data.instantBuild then
            buildProgress=math.clamp(
                (tick()-data.startedAt)/BUILD_TIME,
                0,
                1
            )
        end

        local visibleDistance=fullDistance*buildProgress

        if visibleDistance<=0.01 then
            clearAimingParts(char)
            return
        end
        ...
        local count=math.max(
            1,
            math.ceil(
                visibleDistance/(DASH_LENGTH+GAP_LENGTH)
            )
        )

        local parts=ensureAimingParts(
            char,
            count
        )
        ...
        while traveled<visibleDistance-0.01
        and used<count do
            local startDistance=traveled
            local endDistance=math.min(
            ...
            local part=parts[used]

            part.Color=data.yellow and YELLOW or WHITE
            ...
            traveled=endDistance+GAP_LENGTH
        end

        for i=used+1,#parts do
            hidePart(parts[i])
        end
    end

    local function raycastGround(char,origin)
        local params=RaycastParams.new()
        params.FilterType=Enum.RaycastFilterType.Exclude
        params.FilterDescendantsInstances={char}
        params.IgnoreWater=true
    ...
    local function ensureHeavenParts(char,count)
        local list=heavenParts[char] or {}
    ...
    local function updateHeavenEyes(char)
        if not STATE.heavenEyesEnabled
        or not char
        or not char.Parent then
        ...
        -- Heaven Eyes is a single red, curved triangle drawn on the ground
        -- in front of the killer. It is deliberately independent of the
        -- player's position and does not follow the player's Humanoid.
        local origin=root.Position
        local forward=Vector3.new(
            head.CFrame.LookVector.X,
            0,
            head.CFrame.LookVector.Z
        )
        ...
        local groundOrigin=raycastGround(
            char,
            origin+Vector3.new(0,4,0)
        )

        local apex=groundOrigin+forward*1.2
        local leftEnd=groundOrigin
            +forward*HEAVEN_DISTANCE
            -right*(HEAVEN_DISTANCE*0.72)

        local rightEnd=groundOrigin
            +forward*HEAVEN_DISTANCE
            +right*(HEAVEN_DISTANCE*0.72)

        local parts=ensureHeavenParts(
            char,
            HEAVEN_SEGMENTS*3
        )

        local function curvedEdge(a,b,bulge,baseIndex)
            local previous=a

            for segment=1,HEAVEN_SEGMENTS do
                local t=segment/HEAVEN_SEGMENTS
                local point=a:Lerp(b,t)

                local curve=math.sin(t*math.pi)
                    *bulge

                point=point+right*curve

                local clipped,hit=raycastSegment(
                    char,
                    previous,
                    point
                )

                local part=parts[baseIndex+segment]
                part.Color=RED

                if (clipped-previous).Magnitude>0.01 then
                    pointPart(
                        part,
                        previous,
                        clipped
                ...
                if hit then
                    for rest=segment+1,HEAVEN_SEGMENTS do
                        hidePart(parts[baseIndex+rest])
                    end
                    break
                end
            end
        ...
        curvedEdge(
            leftEnd,
            rightEnd,
            -HEAVEN_CURVE*HEAVEN_DISTANCE*0.55,
            HEAVEN_SEGMENTS
        )
    ...
    local function registerTrack(char,track)
        ...
        local previous=activeTracks[char][track]

        if previous then
            previous.startedAt=tick()
            previous.instantBuild=(id==117886494230451)
            previous.yellow=(colorMode=="yellow")
            return
        end

        activeTracks[char][track]={
            startedAt=tick(),
            yellow=(colorMode=="yellow"),
            instantBuild=(id==117886494230451)
        }
    ...
    local function bindCharacter(char)
        if not char
        or not char.Parent
        or char==localPlayer.Character then
            return
        end
        ...
        if not animator then
            task.spawn(function()
                ...
                if a
                and char.Parent
                and not killerWatchConnections[char] then
                    bindCharacter(char)
                end
            end)
        ...
        local recognizedNow=false

        for _,track in ipairs(animator:GetPlayingAnimationTracks()) do
            local id=numericId(track)

            if id and KILLER_AIMING_ANIMS[id] then
                recognizedNow=true
                break
            end
        end

        if not isKillerCharacter(char) and not recognizedNow then
            return
        end
    ...
    local function bindAll()
        for _,player in ipairs(Players:GetPlayers()) do
            ...
            if player~=localPlayer
            and not player:GetAttribute("KillerAimingBound") then
                player.CharacterAdded:Connect(function(char)
                    task.defer(function()
                        bindCharacter(char)
                    end)
    ...
    local function setEnabled(v)
        ...
        if not v then
            for char in pairs(aimingParts) do
                clearAimingParts(char)
            end
    ...
    RunService.RenderStepped:Connect(function()
        if STATE.killerAimingEnabled then
            ...
            for char in pairs(activeTracks) do
                local data=getCurrentAiming(char)
        ...
        if STATE.heavenEyesEnabled then
            for _,player in ipairs(Players:GetPlayers()) do
                ...
                and isKillerCharacter(player.Character) then
