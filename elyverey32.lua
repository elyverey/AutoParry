...
local KillerAimingFeature=(function()
    ...
    local function createLine(store,char,color,prefix)
        ...
        -- Reference movement/orientation from HumanoidRootPart, but the
        -- actual visual origin MUST be the exact center of the Head.
        -- Each feature owns its own attachment, so LOS/Always never share
    ...
    local function updateLine(data,char)
        ...
        -- RootPart is the reference for movement/orientation, while the
        -- Beam source stays physically attached to the current Head.
        -- This keeps the visible line centered on the head without making
        -- the line itself a RootPart line.
        if data.startAttachment.Parent ~= head then
            data.startAttachment.Parent=head
        ...
        -- Always calculate the endpoint from the Killer's CURRENT position.
        -- Never keep a cached world-space endpoint.
        ...
        -- Explicitly refresh both world positions every frame.  The source
        -- is the exact center of Head; RootPart only supplies direction.
        data.startAttachment.WorldPosition=head.Position
        data.endpointAttachment.WorldPosition=endpoint
    ...
    local function createAlwaysLine(char)
        ...
        -- A small neon sphere sits exactly on the end of the Always Line.
        ...
        ball.Size=Vector3.new(LINE_THICKNESS*2.2,LINE_THICKNESS*2.2,LINE_THICKNESS*2.2)
        ball.Color=WHITE
        ball.Transparency=0
        ball.Parent=Workspace
        data.endpointBall=ball

        return data
    end

    local function createAlwaysYellowLine(char)
        -- Kept as a compatibility wrapper; Always Line now changes color
        -- directly instead of creating a second yellow line.
        return nil
    end
    ...
    local function updateAlwaysLine(char)
        ...
        local yellow=hasActiveYellowAnimation(char)
        local color=yellow and YELLOW or WHITE
        ...
        -- No second Beam: yellow replaces the white Always Line while the
        -- lunge/corrupt animation is active, then automatically returns white.
        clearLine(alwaysYellowLines,char)
    end
