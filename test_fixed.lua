...
do
    ...
    local function getMaskedDirectionalPrediction(char)
        ...
        local distance=Vector3.new(relative.X,0,relative.Z).Magnitude

        if distance<=0.05 then
            return 0
        end

        local direction=Vector3.new(relative.X,0,relative.Z).Unit
        local closingSpeed=math.max(0,relativeVelocity:Dot(direction))

        local ping=math.clamp(localPlayer:GetNetworkPing(),0,0.3)
        local base=ping+0.04
        local best=distance

        local samples={base,base+0.04,base+0.08,base+0.12,base+0.18,base+0.25,base+0.32,base+0.40}
    ...
    local function isFacingLocalPlayer(killerChar)
        ...
        if dot>=math.max(0,threshold) then
            return true
        end
        ...
        if flatVelocity.Magnitude>6 then
            ...
            local aimThreshold=0.1+((aimStrictness-0.1)/2.9)*0.8

            if velocityDot>=math.max(threshold-0.08,aimThreshold) then
                return true
            end
        end
    ...
    local function hiddenCanParry(char,seq)
        ...
        if STATE.ignoredTheHidden or seq.consumed or not seq.armed or not seq.skillFinished then
            return false
        end
    ...
    local function registerHiddenWalk(char,track)
        ...
        if not seq or seq.consumed or not seq.armed or not seq.skillFinished then
            return
        end
    ...
    local function registerSkillAttack(char,kName,track)
        ...
        if oldSeq.skillTrack==track then
            return
        end
    ...
    local function maskedCanParryNow(char,seq)
        ...
        return getMaskedPredictedDistance(char)<=STATE.autoParryRadius
    end
