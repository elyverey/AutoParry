...
do
    ...
    local function getPalletModelFromHit(hit)
        if not hit then
            return nil
        end
        ...
        for _=1,6 do
            ...
            if current:IsA("Model") and current.Name:lower():find("pallet") then
                return current
            end
        ...
        if hit.Name:lower():find("pallet") then
            return hit
        end
    ...
    local function isStandingPallet(pallet)
        if not pallet or not pallet.Parent then
            return false
        end

        local ok,size=pcall(function()
            return pallet:GetExtentsSize()
        end)
        ...
        -- A placed pallet is substantially taller than its thickness.
        -- A dropped pallet is mostly flat against the floor.
        local horizontal=math.max(size.X,size.Z)
        local vertical=size.Y

        return vertical>=1.25 and vertical>=horizontal*0.32
    end
    ...
    local function getObstacleState(origin,target,extraIgnore)
        ...
        local palletStanding=false
        local palletDropped=false

        for rayIndex,pair in ipairs(rayData) do
            local result=Workspace:Raycast(pair[1],pair[2]-pair[1],params)

            if rayIndex==1 then
                directBlocked=result~=nil
                directHit=result and result.Instance or nil
            end
            ...
            if not result then
                ...
                if hit then
                    ...
                    local pallet=getPalletModelFromHit(hit)
                    if pallet then
                        palletHit=true

                        if isStandingPallet(pallet) then
                            palletStanding=true
                        else
                            palletDropped=true
                        end
                    elseif n:find("plank") then
                        palletHit=true
                    end
                end
            end
        ...
        -- If even one side/corner ray gets through while another ray is blocked,
        -- treat it as an edge threat. This covers pipes, corners, props, walls,
        -- and other objects whose collision can still be reached around an edge.
        local edgeThreat=blockedCount>0 and clearCount>0
    ...
    local function getFaceKillerThreshold()
        local strictness=math.clamp(STATE.faceKillerSensitivity,-0.1,1)
    ...
    local function isFacingLocalPlayer(killerChar)
        if STATE.faceKillerSensitivity<=-0.1 then
            return true
        end
    ...
    local function canNormalAttackParry(killerChar,distance,closingSpeed,predictedDistance)
        ...
        -- Check geometry before the facing test so obstacle cases are handled
        -- independently of the killer's facing direction.
        ...
        -- A placed/standing pallet remains a valid parry situation even when
        -- the attack is coming through the pallet side.
        if obstacle.palletStanding then
            return true
        end

        -- A dropped pallet between the player and killer blocks the opposite-side
        -- parry. This uses the direct attack path, not the side/corner rays.
        if obstacle.palletDropped and obstacle.directBlocked then
            return false
        end

        -- An edge threat is valid only when the direct route is blocked and a
        -- side/corner route is genuinely open. This avoids making every
        -- wall/vault/prop into a universal parry trigger.
        if obstacle.edgeThreat and obstacle.directBlocked then
            return true
        end
        ...
        if state then
            ...
            espLoop=RunService.RenderStepped:Connect(function()
                ...
                if pulseAt>0 then
                    ...
                    elseif elapsed<PARRY_COOLDOWN then
                        -- Keep the visual locked at zero for the entire 60s cooldown.
                        radius=0
                    else
                        -- Only after the full cooldown, smoothly restore the visual.
                        local RESTORE_TIME=1.25
                        local t=math.clamp(
                            (elapsed-PARRY_COOLDOWN)/RESTORE_TIME,
                            0,
                            1
                        )
...
local KillerAimingFeature=(function()
        ...
        for _,player in ipairs(Players:GetPlayers()) do
            if player~=localPlayer and player.Character and player.Character.Parent then
                ...
                and known.name~="Unknown Killer" then
                    -- Keep a previously confirmed Killer until the character leaves.
    ...
    local function updateAlwaysLine(char)
        updateLine(alwaysLines[char],char)
    end
    ...
    local function registerTrack(char,track)
        ...
        or not char.Parent
        or not track
        or not track.Animation
        or not isKillerCharacter(char) then
            return
        end

        local id=numericId(track)
        local color=id and KILLER_AIMING_ANIMS[id]
        if not color then
            return
        end

        activeTracks[char]=activeTracks[char] or {}
        activeTracks[char][track]=true

        local beamColor=(color=="yellow") and YELLOW or WHITE
        createAimingLine(char,beamColor)

        local oldConn=trackConnections[track]
        if oldConn then
            oldConn:Disconnect()
        end
    ...
    local function bindAll()
        for _,player in ipairs(Players:GetPlayers()) do
            ...
            if player~=localPlayer and not player:GetAttribute("KillerAimingBound") then
                player.CharacterAdded:Connect(function(char)
                    task.defer(function()
                        bindCharacter(char)
                    end)
