...
do
    ...
    local HIDDEN_SKILL_PARRY_TIME=0.200
    ...
    local function hiddenTrySkillParry(char,seq)
        ...
        local track=seq.skillTrack
        if not track or not track.IsPlaying then
            return false
        end

        -- The Hidden 981: fire the parry exactly after the skill animation
        -- reaches 0.200 TimePosition. This trigger is intentionally tied to
        -- the animation position rather than distance/facing/attack detection.
        local position=track.TimePosition or 0

        if position < HIDDEN_SKILL_PARRY_TIME then
            return false
        end
...
local KillerAimingFeature=(function()
    ...
    local alwaysLines={}
    local alwaysYellowLines={} -- compatibility store; Always Line uses one Beam and recolors it
    local heavenParts={}
    ...
    -- Each visual line owns TWO independent anchored helper parts.
    -- RootPart is used only as the movement/orientation reference.
    -- The visible origin is always the exact center of Head.
    -- This avoids Roblox Attachment/Beam world-position caching issues.
    local function destroyLineData(data)
        ...
        if data.startPart and data.startPart.Parent then
            data.startPart:Destroy()
        end
        ...
        if data.endpointBall and data.endpointBall.Parent then
            data.endpointBall:Destroy()
        end

        if data.linePart and data.linePart.Parent then
            data.linePart:Destroy()
        end
    end
    ...
    local function makeLinePart(name)
    ...
    local function createLine(store,char,color,prefix)
        ...
        local startPart=makeLinePart(prefix.."StartPart")
        local endpointPart=makeLinePart(prefix.."Endpoint")

        startPart.CFrame=CFrame.new(head.Position)
    ...
    local function updateLine(data,char)
        ...
        -- HumanoidRootPart is the reference for direction.
        -- Head is the actual visual origin: EXACTLY the middle of the head.
        data.sourceRoot=root
        data.sourceHead=head
        ...
        local lineLength=tonumber(data.length) or MAX_LENGTH
        lineLength=math.clamp(lineLength,10,100)

        local origin=head.Position
        local endpoint=origin + direction*lineLength

        -- Move BOTH helper parts every frame.
        -- Nothing is cached in world space.
        data.startPart.CFrame=CFrame.new(origin)
        data.endpointPart.CFrame=CFrame.new(endpoint)

        if data.endpointBall and data.endpointBall.Parent then
            data.endpointBall.CFrame=CFrame.new(endpoint)
        end
    ...
    local function createAlwaysLine(char)
        ...
        -- Always Line is intentionally a standalone physical Neon line.
        -- It does not share Beam/Attachment state with LOS or Heaven Eye.
        local linePart=Instance.new("Part")
        linePart.Name="LineOfSightAlwaysPhysical"
        ...
        linePart.Size=Vector3.new(LINE_THICKNESS,LINE_THICKNESS,10)
        linePart.Color=WHITE
        linePart.Parent=Workspace

        local ball=Instance.new("Part")
        ball.Name="LineOfSightAlwaysEnd"
        ...
        local ballDiameter=LINE_THICKNESS*2.8
    ...
    local function updateAlwaysLine(char)
        ...
        if direction.Magnitude<=0.001 then
            local hd=head.CFrame.LookVector
            direction=Vector3.new(hd.X,0,hd.Z)
        end
        if direction.Magnitude<=0.001 then
            return
        end
        ...
        -- EXACTLY the center of Head is the visible origin.
        local origin=head.Position
        local endpoint=origin + direction*data.length
        local delta=endpoint-origin
        local length=delta.Magnitude
        local midpoint=(origin+endpoint)*0.5

        local color=hasActiveYellowAnimation(char) and YELLOW or WHITE
        data.color=color

        if data.linePart and data.linePart.Parent then
            data.linePart.Color=color
            data.linePart.Size=Vector3.new(LINE_THICKNESS,LINE_THICKNESS,length)
            data.linePart.CFrame=CFrame.lookAt(midpoint,endpoint)
        end

        if data.endpointBall and data.endpointBall.Parent then
            data.endpointBall.Color=color
            data.endpointBall.CFrame=CFrame.new(endpoint)
        end
    end
    ...
    local function pointPart(part,a,b)
        ...
        part.Transparency=0
        part.Size=Vector3.new(.09,.09,length)
        part.CFrame=CFrame.lookAt((a+b)*.5,b)
    end

local function heavenGroundPoint(char,point)
        -- Keep Heaven Eye at the actual floor height of the Killer.
        -- All points use the same floor Y so the shape stays level.
        ...
        local origin=Vector3.new(root.Position.X, root.Position.Y+6, root.Position.Z)
        local hit=workspace:Raycast(origin,Vector3.new(0,-30,0),params)

        local floorY
        if hit then
            floorY=hit.Position.Y+0.08
        else
            local humanoid=char:FindFirstChildOfClass("Humanoid")
            floorY=root.Position.Y-(humanoid and humanoid.HipHeight or 2)
        end

        return Vector3.new(point.X,floorY,point.Z)
    end
    ...
    local function ensureHeavenParts(char)
        ...
        -- 10 points make a smooth curved arc; two extra parts connect
        -- both ends of the arc back to the Killer.
        list={}
        for i=1,11 do
            list[#list+1]=makeHeavenPart("HeavenEyesCurve")
        end
    ...
    local function updateHeavenEyes(char)
        ...
        local head=char:FindFirstChild("Head")
        local center=(head and head.Position) or root.Position
        ...
        --              KILLER
        --
        -- Total arc width = 15.5 studs.
        local halfWidth=HEAVEN_WIDTH*0.5
        local apex=heavenGroundPoint(
            char,
            center+forward*1.0
        )
        ...
        for i=0,arcCount-1 do
            local t=i/(arcCount-1)
            local x=-halfWidth+(HEAVEN_WIDTH*t)

            -- Shallow curved top: the center sits farther forward,
            -- producing the rounded/fan-like shape from the reference.
            local normalized=math.abs((x/halfWidth))
            local depth=12.5+3.5*(1-(normalized^1.65))

            local point=center
                +forward*depth
                +right*x

            arcPoints[#arcPoints+1]=heavenGroundPoint(char,point)
        end

        local parts=ensureHeavenParts(char)

        -- Two straight legs from the Killer to each end of the curved arc.
        pointPart(parts[1],apex,arcPoints[1])

        -- Curved top.
        for i=1,#arcPoints-1 do
            pointPart(parts[i+1],arcPoints[i],arcPoints[i+1])
        end

        pointPart(parts[11],arcPoints[#arcPoints],apex)
    ...
    local function registerTrack(char,track)
        ...
        -- Fallback to AnimationTrack.Name because some clients expose the
        -- track name even when the AnimationId is wrapped/replicated late.
        if not colorMode then
            local n=string.lower(tostring(track.Name or ""))
            if n=="idle1" or n=="idle2"
            or n=="idlespearon1" or n=="idlespearon2"
            or n=="takeoutspear1" or n=="takeoutspear2" then
                colorMode="white"
            elseif n=="spearlunge1" or n=="spearlunge2"
            or n=="spearlunge1op" or n=="spearlunge2op"
            or n=="corrupt" then
                colorMode="yellow"
            end
        end
        ...
        -- Line of Sight is independent from Always Line.
        -- Only build its beams when the LOS toggle is actually enabled.
        if STATE.killerAimingEnabled then
            -- White animation line is the permanent base.
            if not aimingLines[char] then
                createAimingLine(char)
            end

            -- Lunge/Corrupt gets a clearly visible yellow overlay.
            if hasActiveYellowAnimation(char) then
                if not yellowAimingLines[char] then
                    createYellowAimingLine(char)
                end
        ...
        -- Always Line has its own permanent white beam and only changes
        -- that beam to yellow while a yellow animation is active.
        if lineAlways then
    ...
    -- Always Line uses a fixed 115-stud length. The old UI length control is intentionally removed.\n    local function setAlwaysLineLength(_)\n        STATE.alwaysLineLength=MAX_LENGTH\n        local killer=getCurrentKiller()\n        if lineAlways and killer then\n            if not alwaysLines[killer] then\n                createAlwaysLine(killer)\n            end\n            local data=alwaysLines[killer]\n            if data then data.length=MAX_LENGTH end\n            pcall(updateAlwaysLine,killer)\n        end\n    end\n\n    local function setHeavenEyes(v)
    ...
    RunService.RenderStepped:Connect(function()
            ...
            for _,player in ipairs(Players:GetPlayers()) do
                if player.Character then
                    ...
                    if isKillerCharacter(player.Character) then
                        scanCurrentTracks(player.Character)
                    end
                end
            end
        end
        ...
        if STATE.killerAimingEnabled then
            ...
            if killer and killer.Parent then
                -- Re-scan the current Killer's Animator every frame so LOS
                -- cannot get stuck when AnimationPlayed fires before the
                -- replicated AnimationId/Name is available.
                scanCurrentTracks(killer)
                if hasActiveRecognizedAnimation(killer) then
                    if not aimingLines[killer] then
                        createAimingLine(killer)
                    end
                    if hasActiveYellowAnimation(killer) then
                        if not yellowAimingLines[killer] then
                            createYellowAimingLine(killer)
                        end
                    else
                        clearLine(yellowAimingLines,killer)
                    end
                    pcall(updateAimingLine,killer)
                else
                    clearAimingLine(killer)
                end
            end
