...
local KillerAimingFeature=(function()
    ...
    local function createAlwaysYellowLine(char)
        return createLine(alwaysYellowLines,char,YELLOW,"LineOfSightAlwaysYellow")
    end
    ...
    local function ensureHeavenParts(char)
        ...
        list={
            makeHeavenPart("HeavenEyesCurve"),
            makeHeavenPart("HeavenEyesCurve"),
            makeHeavenPart("HeavenEyesCurve"),
            makeHeavenPart("HeavenEyesSideLeft"),
            makeHeavenPart("HeavenEyesSideRight")
        }
    ...
    local function updateHeavenEyes(char)
        ...
        local curveDistance=HEAVEN_DISTANCE
        local apexBase=center+forward*1.0
        local leftBase=center+forward*curveDistance-right*HEAVEN_WIDTH
        local rightBase=center+forward*curveDistance+right*HEAVEN_WIDTH
        ...
        -- Heaven Eye only: keep the compact curved/angled guide, and add
        -- two long straight side rails.  These rails do not affect LOS/Always.
        local sideLength=math.max(10,math.min(100,HEAVEN_DISTANCE*2.35))
        local sideStartOffset=1.5
        local leftSideStart=heavenGroundPoint(
            char,
            center+forward*sideStartOffset-right*(HEAVEN_WIDTH*0.72)
        )
        local rightSideStart=heavenGroundPoint(
            char,
            center+forward*sideStartOffset+right*(HEAVEN_WIDTH*0.72)
        )
        local leftSideEnd=heavenGroundPoint(
            char,
            center+forward*sideLength-right*(HEAVEN_WIDTH*0.72)
        )
        local rightSideEnd=heavenGroundPoint(
            char,
            center+forward*sideLength+right*(HEAVEN_WIDTH*0.72)
        )
        ...
        pointPart(parts[4],leftSideStart,leftSideEnd)
        pointPart(parts[5],rightSideStart,rightSideEnd)
    ...
    local function registerTrack(char,track)
        ...
        if not colorMode then
            ...
            if n=="takeoutspear1" or n=="takeoutspear2"
            or n=="idlespearon1" or n=="idlespearon2" then
                colorMode="white"
            elseif n=="spearlunge1" or n=="spearlunge2"
            or n=="spearlunge1op" or n=="spearlunge2op"
            or n=="corrupt" or n=="lungehold" then
                colorMode="yellow"
            end
        end
