...
do
    ...
    local function isFacingLocalPlayer(killerChar)
        ...
        if flatVelocity.Magnitude>6 then
            ...
            local aimStrictness=math.clamp(STATE.aimStrictness,0.1,3)
            local aimPercent=0.01+((aimStrictness-0.1)/2.9)*0.29
            local velocityRelax=0.10+((aimStrictness-0.1)/2.9)*0.10
...
local KillerAimingFeature=(function()
    ...
    local function isKillerCharacter(char)
        ...
        if player then
            ...
            if detectedName or teamKiller then
        ...
        if lineAlways then
            for _,player in ipairs(Players:GetPlayers()) do
                if player~=localPlayer and player.Character and isKillerCharacter(player.Character) then
                    ...
                    local line=alwaysLines[char]
                    if not line then
                        line=createAlwaysLine(char)
                    end
