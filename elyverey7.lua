...
do
    ...
    local function doParry(force,killerChar,distance,allowPredictedRange)
        ...
        if killerChar and distance then
            ...
            if obstacle.blocked or obstacle.directBlocked or not obstacle.clear then
                return false
            end
        end
    ...
    local function isFacingLocalPlayer(killerChar)
        ...
        local strictnessPercent=(aimStrictness-0.1)/2.9
        local aimToleranceDegrees=27-(strictnessPercent*18)
        local baseAngle=math.deg(math.acos(math.clamp(math.max(0,threshold),-1,1)))
        local allowedAngle=math.min(90,baseAngle+aimToleranceDegrees+12)
        ...
        if flatVelocity.Magnitude>6 then
            ...
            local aimStrictness=math.clamp(STATE.aimStrictness,0.1,3)
            local strictnessPercent=(aimStrictness-0.1)/2.9
            local aimPercent=0.30-(strictnessPercent*0.20)
            local velocityRelax=0.18-(strictnessPercent*0.08)
            local velocityAimThreshold=math.max(
                0.05,
                1-(aimPercent*2.5)-velocityRelax
            )
...
local KillerDetectorGUI=nil
local KillerDetectorRows=nil
local KillerDetectorStatus=nil
local KillerDetectorSelected=nil

local function copyToClipboard(textValue)
    if typeof(setclipboard)=="function" then
        pcall(setclipboard,textValue)
        return true
    end
    if typeof(toclipboard)=="function" then
        pcall(toclipboard,textValue)
        return true
    end
    return false
end

local function makeDetectorText(item)
    if not item then
        return "No direct killer detected"
    end
    return string.format("%s | %s | score %d",
        tostring(item.player and item.player.Name or "?"),
        tostring(item.name or "Killer"),
        tonumber(item.score) or 0)
end

local function refreshKillerDetectorGUI()
    if not KillerDetectorGUI or not KillerDetectorGUI.Parent then
        return
    end

    local best,candidates=scanDirectKiller()

    if KillerDetectorRows then
        for _,child in ipairs(KillerDetectorRows:GetChildren()) do
    ...
    if best then
        KillerDetectorSelected=best
        KillerDetectorStatus.Text="Detected: "..makeDetectorText(best)
    else
        KillerDetectorSelected=nil
        KillerDetectorStatus.Text="Detected: NONE"
    end
    ...
    if #candidates==0 then
        ...
        empty.TextColor3=Color3.fromRGB(190,190,190)
        empty.Font=Enum.Font.Gotham
        empty.TextSize=12
        empty.Parent=KillerDetectorRows
        return
    end
    ...
    for _,item in ipairs(candidates) do
        ...
        row.BackgroundColor3=(best and item.player==best.player)
            and Color3.fromRGB(55,75,55)
            or Color3.fromRGB(35,35,35)
        row.BorderSizePixel=0
        row.TextXAlignment=Enum.TextXAlignment.Left
        row.TextYAlignment=Enum.TextYAlignment.Top
        row.TextWrapped=true
        row.Text="  "..makeDetectorText(item).."\n  "..table.concat(item.evidence," | ")
        row.TextColor3=Color3.fromRGB(235,235,235)
        row.Font=Enum.Font.Gotham
        row.TextSize=11
        row.Parent=KillerDetectorRows

        row.MouseButton1Click:Connect(function()
            KillerDetectorSelected=item
            KillerDetectorStatus.Text="Selected: "..makeDetectorText(item)
        end)
    end

    KillerDetectorRows.CanvasSize=UDim2.new(0,0,0,#candidates*56+8)
end

local function createKillerDetectorGUI()
    if KillerDetectorGUI and KillerDetectorGUI.Parent then
        KillerDetectorGUI.Enabled=not KillerDetectorGUI.Enabled
        if KillerDetectorGUI.Enabled then
            refreshKillerDetectorGUI()
        end
        return
    end

    local gui=Instance.new("ScreenGui")
    gui.Name="Elyverey_KillerDetector"
    gui.ResetOnSpawn=false
    gui.ZIndexBehavior=Enum.ZIndexBehavior.Sibling
    gui.Parent=PlayerGui
    KillerDetectorGUI=gui

    local frame=Instance.new("Frame")
    frame.Name="Main"
    frame.Size=UDim2.fromOffset(360,330)
    ...
    KillerDetectorStatus=Instance.new("TextLabel")
    KillerDetectorStatus.Size=UDim2.new(1,-16,0,38)
    KillerDetectorStatus.Position=UDim2.fromOffset(8,37)
    KillerDetectorStatus.BackgroundColor3=Color3.fromRGB(30,30,30)
    KillerDetectorStatus.Text="Press Scan"
    KillerDetectorStatus.TextWrapped=true
    KillerDetectorStatus.TextXAlignment=Enum.TextXAlignment.Left
    KillerDetectorStatus.TextColor3=Color3.fromRGB(220,220,220)
    KillerDetectorStatus.Font=Enum.Font.Gotham
    KillerDetectorStatus.TextSize=11
    KillerDetectorStatus.Parent=frame

    local scan=Instance.new("TextButton")
    scan.Size=UDim2.fromOffset(90,30)
    scan.Position=UDim2.new(1,-198,0,82)
    scan.BackgroundColor3=Color3.fromRGB(55,55,55)
    scan.Text="SCAN"
    scan.TextColor3=Color3.new(1,1,1)
    scan.Font=Enum.Font.GothamBold
    scan.TextSize=12
    scan.Parent=frame
    scan.MouseButton1Click:Connect(refreshKillerDetectorGUI)

    local copy=Instance.new("TextButton")
    copy.Size=UDim2.fromOffset(90,30)
    copy.Position=UDim2.new(1,-100,0,82)
    copy.BackgroundColor3=Color3.fromRGB(55,55,55)
    copy.Text="COPY"
    ...
    copy.MouseButton1Click:Connect(function()
        local item=KillerDetectorSelected
        if not item then
            local best=scanDirectKiller()
            item=best
        end
        if item then
            ...
            if copyToClipboard(payload) then
                KillerDetectorStatus.Text="Copied: "..makeDetectorText(item)
            else
                KillerDetectorStatus.Text="Clipboard function unavailable"
            end
        end
    end)

    KillerDetectorRows=Instance.new("ScrollingFrame")
    KillerDetectorRows.Size=UDim2.new(1,-16,1,-122)
    KillerDetectorRows.Position=UDim2.fromOffset(8,118)
    KillerDetectorRows.BackgroundColor3=Color3.fromRGB(18,18,18)
    KillerDetectorRows.BorderSizePixel=0
    KillerDetectorRows.ScrollBarThickness=5
    KillerDetectorRows.CanvasSize=UDim2.new(0,0,0,0)
    KillerDetectorRows.Parent=frame

    local layout=Instance.new("UIListLayout")
    layout.Padding=UDim.new(0,4)
    layout.Parent=KillerDetectorRows
...
local function BuildUI()
    ...
    AimSection:AddButton({
        Title="Killer Detector",
        Callback=function()
            createKillerDetectorGUI()
        end
    })
