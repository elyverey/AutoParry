...
do
    ...
    local function doParry(force,killerChar,distance,allowPredictedRange)
        ...
        if killerChar and distance then
            ...
            if obstacle.droppedPallet
            and obstacle.palletBlockedCount>=4
            and obstacle.blockedCount>=4
            and not obstacle.clear then
                return false
            end

            -- Standing pallets do not suppress a valid attack. The player can
            -- still be hit through an exposed edge/corner of the object.
        end
...
local KillerAimingFeature=(function()
    ...
    local function updateAlwaysLine(char)
        ...
        local color=getAlwaysLineColor(char)
