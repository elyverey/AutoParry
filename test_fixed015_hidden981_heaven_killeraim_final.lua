...
do
    ...
    local function isFacingLocalPlayer(killerChar)
        ...
        local aimStrictness=math.clamp(STATE.aimStrictness,0.1,3)
        local aimPercent=0.01+((aimStrictness-0.1)/2.9)*0.29
        local aimToleranceDegrees=90*aimPercent
        local baseAngle=math.deg(math.acos(math.clamp(math.max(0,threshold),-1,1)))
        local allowedAngle=math.min(90,baseAngle+aimToleranceDegrees)
        local aimThreshold=math.cos(math.rad(allowedAngle))

        if dot>=aimThreshold then
            return true
        end
        ...
        if flatVelocity.Magnitude>6 then
            ...
            local aimStrictness=math.clamp(STATE.aimStrictness,0.1,3)
            local aimPercent=0.01+((aimStrictness-0.1)/2.9)*0.29
            local velocityAimThreshold=math.max(
                0.15,
                1-(aimPercent*2.5)
            )

            if velocityDot>=math.max(threshold-0.08,velocityAimThreshold) then
                return true
            end
        end
...
local KillerAimingFeature=(function()
    ...
    local HEAVEN_DISTANCE=12
    local HEAVEN_HEIGHT=0.22
    local HEAVEN_WIDTH=7.5
    ...
    local function updateAiming(char)
        ...
        local data=persistentAim[char]

        if not data then
            return
        end

        -- The line persists after the triggering animation ends.
        -- Its origin stays at the center of the R6 head.
        -- Only the viewing direction is updated, so when the Killer turns,
        -- the line turns with the Killer's facing direction without
        -- rebuilding, disappearing, or being tied to the animation track.
        ...
        local endPoint=origin+direction*MAX_LENGTH
        local parts=ensureAimingParts(char,1)

        local part=parts[1]
        part.Color=data.color
        pointPart(part,origin,endPoint)
    ...
    local function updateHeavenEyes(char)
        ...
        -- Heaven Eyes:
        -- One single white triangle on the ground, slightly raised above it.
        -- It follows only the killer's facing direction.
        -- No raycast clipping and no automatic disappearance.
        ...
        local center=root.Position+Vector3.new(0,HEAVEN_HEIGHT,0)

        local apex=center+forward*1.0
        local leftEnd=center
            +forward*HEAVEN_DISTANCE
            -right*HEAVEN_WIDTH

        local rightEnd=center
            +forward*HEAVEN_DISTANCE
            +right*HEAVEN_WIDTH

        local parts=ensureHeavenParts(char,3)
    ...
    local function registerTrack(char,track)
        ...
        -- These exact Killer animations are sufficient to identify the
        -- character for Killer Aiming. Do not depend on KillerName/team
        -- detection before creating the line.
        if not colorMode then
            return
        end

        if not isKillerCharacter(char) then
            STATE.killerCharacters[char]=STATE.killerCharacters[char] or {
                name="Animation Killer",
                model=char
            }
        end
        ...
        if previous then
            ...
            local existing=persistentAim[char]

            if existing then
                existing.color=(colorMode=="yellow" and YELLOW or WHITE)
            else
                local head=char:FindFirstChild("Head")
                if head then
                    persistentAim[char]={
                        direction=head.CFrame.LookVector,
                        color=(colorMode=="yellow" and YELLOW or WHITE)
                    }
                end
            end
        ...
        if not persistentAim[char] then
            local head=char:FindFirstChild("Head")
            ...
            -- Keep the original direction fixed, but allow the active
            -- animation family to change the line color.
            persistentAim[char].color=
                (colorMode=="yellow" and YELLOW or WHITE)
        end

        -- Animation playback is only the trigger for creating/updating
        -- the persistent line. Stopping the animation must never remove it.
        local conn
        conn=track.Stopped:Connect(function()
            trackConnections[track]=nil
    ...
    RunService.RenderStepped:Connect(function()
        if STATE.killerAimingEnabled then
            ...
            for char in pairs(persistentAim) do
                if not char
                or not char.Parent then
                    persistentAim[char]=nil
                    clearAimingParts(char)
