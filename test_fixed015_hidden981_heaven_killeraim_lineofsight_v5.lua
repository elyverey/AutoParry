...
do
    ...
    local function getAimStrictnessDistance()
        local strictness=math.clamp(STATE.aimStrictness,0.1,3)
        local radius=math.max(STATE.autoParryRadius,0)
        if radius<=0 or strictness<=0.1 then
            return 0
        end
        return ((strictness-0.1)/2.9)*radius
    end

    local function getKillerDirection(killerChar)
        ...
        if not killerRoot or not killerHumanoid then
            return nil
        end

        local moveDirection=killerHumanoid.MoveDirection
        ...
        if flatMove.Magnitude<=0.05 then
            ...
            flatMove=Vector3.new(look.X,0,look.Z)
        end
    ...
    local function getKillerFacingDot(killerChar)
        local localChar=localPlayer.Character
        local localRoot=localChar and localChar:FindFirstChild("HumanoidRootPart")
        local killerRoot=killerChar and killerChar:FindFirstChild("HumanoidRootPart")
        if not localRoot or not killerRoot then
        ...
        local killerDirection=getKillerDirection(killerChar)
        if not killerDirection then
            return nil,distance
        end

        return killerDirection:Dot(flatOffset.Unit),distance
    end
    ...
    local function isFacingLocalPlayer(killerChar)
        ...
        local forwardDot,distance=getKillerFacingDot(killerChar)
        if forwardDot==nil then
            return false
        end
        ...
        -- Parry Radius is always the outer/main boundary.
        if distance>STATE.autoParryRadius then
            return false
        end

        -- Never allow the area behind the Killer.
        if forwardDot<=0 then
            return false
        end

        -- Face Killer Sensitivity controls how directly the Killer must be
        -- moving toward the LocalPlayer.
        local faceThreshold=math.clamp(STATE.faceKillerSensitivity,-1,1)
        if forwardDot<faceThreshold then
            return false
        end

        -- Aim Strictness controls the accepted distance inside Parry Radius.
        -- 0.1 => 0 studs, 3.0 => Parry Radius.
        local strictnessDistance=getAimStrictnessDistance()
        return distance<=strictnessDistance
    end

    local function canNormalAttackParry(killerChar,distance,closingSpeed,predictedDistance)
        if STATE.faceKillerSensitivity<=-1 then
            return distance<=STATE.autoParryRadius
        end

        -- Parry Radius is the primary/outer boundary.
        if distance>STATE.autoParryRadius then
            return false
        end

        local forwardDot=getKillerFacingDot(killerChar)
        if forwardDot==nil then
            return false
        end
        ...
        -- Aim Strictness scales with the current Parry Radius.
        local strictnessDistance=getAimStrictnessDistance()
        return distance<=strictnessDistance
    end
...
local KillerAimingFeature=(function()
    ...
    local HEAVEN_DISTANCE=15
    local HEAVEN_HEIGHT=0.22
    local HEAVEN_WIDTH=6.5
    ...
    local function isKillerCharacter(char)
        ...
        for _,player in ipairs(Players:GetPlayers()) do
            if player~=localPlayer and player.Character==char then
                ...
                if killerName and killerName:IsA("StringValue") and killerName.Value~="" then
                    return true
                end
    ...
    local function createAimingLine(char,color)
        ...
        endpointPart.Name="LineOfSightEndpoint"
    ...
    local function makeHeavenPart(name)
        ...
        part.Size=Vector3.new(.055,.055,.1)
    ...
    local function pointPart(part,a,b)
        ...
        part.Size=Vector3.new(.055,.055,length)
        ...
        if lineAlways then
            for _,player in ipairs(Players:GetPlayers()) do
                if player~=localPlayer and player.Character and isKillerCharacter(player.Character) then
                    ...
                    local line=aimingLines[char]
                    if not line then
                        line=createAimingLine(char,WHITE)
                    end
                    updateAimingLine(char)
                end
            end
            ...
            if player~=localPlayer and player.Character then
                ...
                if isKillerCharacter(char) then
                    if lineAlways then
                        ...
                        updateAimingLine(char)
                    elseif not hasActiveRecognizedAnimation(char) then
                        clearAimingLine(char)
                    end
                end
            end
        ...
        if STATE.killerAimingEnabled then
            for _,player in ipairs(Players:GetPlayers()) do
                if player~=localPlayer and player.Character then
                    bindCharacter(player.Character)
                    local char=player.Character
                    if lineAlways and isKillerCharacter(char) then
                        if not aimingLines[char] then
                            createAimingLine(char,WHITE)
                        end
                    end
                end
            end
            ...
            for char in pairs(aimingLines) do
                if not char or not char.Parent then
                    clearAimingLine(char)
                elseif lineAlways or hasActiveRecognizedAnimation(char) then
                    updateAimingLine(char)
                else
                    clearAimingLine(char)
                end
