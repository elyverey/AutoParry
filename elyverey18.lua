...
do
    ...
    local function getObstacleState(origin,target,extraIgnore)
        ...
        local sideOffset=0.48
        local verticalOffset=0.62
        local rayData={
            {origin,target,true},
            ...
            -- Extra body-edge samples. These are deliberately small offsets:
            -- they help attacks around pipe/corner edges without turning a
            -- solid wall/window into a free parry.
            {origin+right*0.75,target+right*0.75,false},
            {origin-right*0.75,target-right*0.75,false},
            {origin+Vector3.new(0,0.85,0),target+Vector3.new(0,0.85,0),false},
            {origin-Vector3.new(0,0.85,0),target-Vector3.new(0,0.85,0),false},
        }
        ...
        local clear=clearCount>=math.max(5,math.ceil(sampleCount*0.66))
        local blocked=blockedCount>=math.ceil(sampleCount*0.50)

        -- A partial clear corridor means the player is near an object's edge.
        -- It is only usable when the killer is actually facing the player and
        -- is close enough for the attack to reach the edge.
        local edgeClear=clearCount>=3 and blockedCount<=math.ceil(sampleCount*0.50)
    ...
    local function doParry(force,killerChar,distance,allowPredictedRange,rangeOverride)
        ...
        if killerChar and distance then
            local allowedRange=rangeOverride or STATE.autoParryRadius
            ...
            local facing=isFacingLocalPlayer(killerChar)
            local edgeRange=STATE.autoParryRadius+1.5

            -- A standing pallet is not treated as a hard anti-parry wall when
            -- the killer is beside it and the attack is actually aimed at us.
            local standingPalletParry=
                obstacle.standingPallet
                and facing
                and distance<=edgeRange

            -- Same treatment for the exposed edge/corner of large map props.
            -- A solid wall/window still requires a real clear corridor.
            local edgeParry=
                obstacle.edgeClear
                and facing
                and distance<=edgeRange
                and not obstacle.vault
                and not obstacle.droppedPallet

            if (obstacle.blocked or obstacle.directBlocked or not obstacle.clear)
            and not standingPalletParry
            and not edgeParry then
                return false
            end
        end
    ...
    local function isFacingLocalPlayer(killerChar)
        ...
        -- Keep both UI controls live while making them less aggressive:
        -- lower Face Killer Sensitivity and lower Aim Strictness both widen
        -- the accepted attack-facing cone.
        local sensitivity=math.clamp(STATE.faceKillerSensitivity,-1,1)
        local strictPercent=(sensitivity+1)/2
        local aimStrict=math.clamp(STATE.aimStrictness,0.1,3)
        local aimPercent=math.clamp(aimStrict/3,0,1)

        -- The old cone was too narrow. This range is deliberately wider.
        local allowedAngle=104-(strictPercent*20)-(aimPercent*16)
        local aimThreshold=math.cos(math.rad(allowedAngle))
        local combinedThreshold=math.min(threshold,aimThreshold)

        if dot>=combinedThreshold then
            return true
        end
    ...
    local function OnParryCharacterAdded(player,char)
        ...
        animator.AnimationPlayed:Connect(function(track)
            ...
            -- Generic attack/lungehold fallback. Register it into the same
            -- activeAttackers queue as known attack IDs. Heartbeat is the
            -- primary detector, so animation detection never directly fires
            -- Parry from AnimationPlayed.
            local trackName=string.lower(tostring(track.Name or ""))
            local animationName=string.lower(tostring(track.Animation.Name or ""))
            local isGenericLungehold=
                trackName:find("lungehold",1,true)~=nil
                or animationName:find("lungehold",1,true)~=nil
            local isGenericAttack=
                trackName:find("attack",1,true)~=nil
                or animationName:find("attack",1,true)~=nil

            local profile=ATTACK_PROFILES[numericId]

            if not profile and (isGenericAttack or isGenericLungehold)
            and sharedIsResolvedKiller(char) then
                local genericLength=track.Length
                if not genericLength or genericLength<=0 then
                    genericLength=isGenericLungehold and 1.50 or 1.00
                end

                profile={
                    type=isGenericLungehold and "lungehold" or "attack",
                    ...
                    or critical then
                        ...
                        if canNormalAttackParry(
                            ...
                            and predictedDistance<=STATE.autoParryRadius+1.75 then
                                trigger=true
                            elseif data.profile.explicitAttack
                            and animationReady
                            and attackWindowOpen
                            and distance<=STATE.autoParryRadius+2.0 then
                                -- Explicitly named attack/lungehold animations
                                -- can trigger while the survivor is stationary;
                                -- obstruction checks remain inside doParry().
                                trigger=true
                            end
    ...
    -- Pulse layer: a very early attack-state accelerator. It is NOT the
    -- primary detector; Heartbeat below owns the normal Auto Parry pipeline.
    -- animation alone; it only wakes the normal distance/velocity/obstacle
    -- checks slightly before the configured attack trigger. This keeps the
    -- existing anti-wall/edge/pallet validation as the final gate.
    local PULSE_LEAD_TIME=0.035
    local PULSE_MIN_INTERVAL=0.010
    local pulseLastTime={}

    local function normalAttackPulseMonitor()
        if not STATE.autoParryEnabled then
            return
        end
        ...
        for killerChar,data in pairs(STATE.activeAttackers) do
            ...
            and data.profile then
                ...
                if killerRoot then
                    ...
                    -- Pulse only around the attack's opening frame. For the
                    -- regular attack profiles this is the earliest safe wake-up;
                    -- lunge profiles may use their tiny configured trigger.
                    local pulseReady=(position+PULSE_LEAD_TIME)>=triggerTime
                        or progress+((PULSE_LEAD_TIME*speed)/math.max(length,0.001))>=triggerProgress

                    if pulseReady then
                        local last=pulseLastTime[killerChar] or 0
                        if now-last>=PULSE_MIN_INTERVAL then
                            pulseLastTime[killerChar]=now

                            local killerVelocity=killerRoot.AssemblyLinearVelocity
                            local killerFlatVelocity=Vector3.new(
                                killerVelocity.X,0,killerVelocity.Z
                            )

                            local localVelocity=localRoot.AssemblyLinearVelocity
                            local localFlatVelocity=Vector3.new(
                                localVelocity.X,0,localVelocity.Z
                            )
                            ...
                            local ping=math.clamp(localPlayer:GetNetworkPing(),0,0.3)
                            local predictionTime=ping+0.025
                            ...
                            -- Pulse is only an EDGE/CORNER accelerator now.
                            -- NormalAttackersMonitor remains the primary detector.
                            -- Pulse is allowed to wake the final parry check only
                            -- when the body corridor is partially blocked/at an
                            -- exposed object edge or standing pallet.
                            local obstacle=getObstacleState(
                                localRoot.Position,
                                killerRoot.Position,
                                killerChar
                            ...
                            if edgeSituation
                            and distance<=STATE.autoParryRadius+1.5
                            and (
                                closingSpeed>=1.0
                                or predictedDistance<=STATE.autoParryRadius+0.75
                            ) then
                                if doParry(
                                    true,
                                    killerChar,
                                    distance,
                                    false,
                                    STATE.autoParryRadius+1.5
                                ) then
                                    data.consumed=true
                                end
                            end
                        end
                    elseif position<=0.02 then
                        -- New animation track: reset its pulse gate so a replay
                        -- of the same attack can be detected immediately.
                        pulseLastTime[killerChar]=0
                    end
                end
            else
                pulseLastTime[killerChar]=nil
            end
        end
    end
    ...
    -- Heartbeat is the PRIMARY detector. All normal, Hidden, Masked and
    -- skill attack checks are driven from this backbone.
    --
    -- PreSimulation only runs the Pulse accelerator. Pulse is intentionally
    -- restricted to edge/corner situations inside normalAttackPulseMonitor().
    -- It never replaces the normal Heartbeat detector.
    RunService.Heartbeat:Connect(function()
        if not STATE.autoParryEnabled then
            return
        end

        -- Primary normal attack detector.
        normalAttackersMonitor()

        -- Skill-specific heartbeat checks.
        normalAttackHeartbeatMonitor()

        -- Killer-specific heartbeat detectors.
        hiddenHeartbeatMonitor()
        maskedHeartbeatMonitor()

        -- Edge/corner accelerator runs AFTER the primary detector so it can
        -- catch attacks that arrive around object boundaries.
        normalAttackPulseMonitor()
    end)
    ...
    if RunService.PreSimulation then
        RunService.PreSimulation:Connect(function()
            if STATE.autoParryEnabled then
                -- EARLY CHECK:
                -- Heartbeat remains the primary Auto Parry loop, but this
                -- lightweight extra pass lets an already-registered attack
                -- reach the same validation pipeline before the next Heartbeat.
                -- All normal distance/velocity/LOS/pallet/wall gates still
                -- apply, so this does not turn PreSimulation into a second
                -- independent Parry system.
                normalAttackersMonitor()

                -- Pulse remains an edge/corner accelerator only.
                normalAttackPulseMonitor()
            end
        end)
    end
...
local KillerAimingFeature=(function()
    ...
    local HEAVEN_DISTANCE=16.5
    local HEAVEN_HEIGHT=0.33
    local HEAVEN_WIDTH=7.0
    ...
    local function updateLine(data,char)
        ...
        local lineLength=data.length or MAX_LENGTH
        data.endpointPart.CFrame=CFrame.new(origin+direction*lineLength)
    ...
    local function clearAimingLine(char)
        ...
        clearLine(yellowAimingLines,char)
    end

    local function createAimingLine(char)
        -- White is the permanent animation base.
        return createLine(aimingLines,char,WHITE,"LineOfSightAnimation")
    end

    local function createYellowAimingLine(char)
        -- Yellow is a second Beam, intentionally layered over the white beam.
        return createLine(
            yellowAimingLines,
            char,
            YELLOW,
            "LineOfSightAnimationYellow"
        )
    end

    local function updateAimingLine(char)
        local white=aimingLines[char]
        if white then
            white.color=WHITE
            setBeamColor(white.beam,WHITE)
            updateLine(white,char)
        end

        local yellow=yellowAimingLines[char]
        if yellow then
            yellow.color=YELLOW
            setBeamColor(yellow.beam,YELLOW)
            updateLine(yellow,char)
        end
    end
    ...
    local function createAlwaysLine(char)
        ...
        if data then
            data.length=alwaysLineLength
        end
        return data
    end

    local function createAlwaysYellowLine(char)
        local data=createLine(alwaysYellowLines,char,YELLOW,"LineOfSightAlwaysYellow")
        if data then
            data.length=alwaysLineLength
        end
        return data
    ...
    local function updateAlwaysLine(char)
        ...
        -- Always Line keeps its white base. During lunge/corrupt a separate
        -- yellow Beam is layered on top of it, exactly like the animation line.
        if hasActiveYellowAnimation(char) then
            local yellow=alwaysYellowLines[char]
            if not yellow then
                yellow=createAlwaysYellowLine(char)
            end
            yellow.length=alwaysLineLength
            yellow.color=YELLOW
            setBeamColor(yellow.beam,YELLOW)
            updateLine(yellow,char)
        else
    ...
    local function makeHeavenPart(name)
        ...
        part.Size=Vector3.new(.045,.045,.1)
    ...
    local function pointPart(part,a,b)
        ...
        part.Size=Vector3.new(.045,.045,length)
    ...
    -- Heaven Eye only: curved arc. This does NOT modify Line of Sight
    -- or Always Line; both of those remain straight Beams.
    local HEAVEN_ARC_SEGMENTS=28
    local HEAVEN_ARC_DEGREES=82

    local function ensureHeavenParts(char)
        local list=heavenParts[char]
        if list and #list==HEAVEN_ARC_SEGMENTS then
            return list
        end
        ...
        list={}
        for i=1,HEAVEN_ARC_SEGMENTS do
            list[i]=makeHeavenPart("HeavenEyesArc")
        end
    ...
    local function updateHeavenEyes(char)
        ...
        -- Keep the existing Heaven Eye range, but draw its front boundary
        -- as a smooth arc instead of a triangle.
        local radius=HEAVEN_DISTANCE
        local halfAngle=math.rad(HEAVEN_ARC_DEGREES/2)
        local center=root.Position

        local points={}
        for i=0,HEAVEN_ARC_SEGMENTS do
            local t=i/HEAVEN_ARC_SEGMENTS
            local angle=-halfAngle+(halfAngle*2*t)
            local dir=(forward*math.cos(angle))+(right*math.sin(angle))
            local point=center+dir*radius
            points[i+1]=heavenGroundPoint(char,point)
        end

        local parts=ensureHeavenParts(char)

        -- Dashed curved arc, similar to the requested reference.
        for i=1,HEAVEN_ARC_SEGMENTS do
            local part=parts[i]
            if (i%2)==1 then
                pointPart(part,points[i],points[i+1])
            else
                part.Transparency=1
            end
            part.Color=WHITE
    ...
    local function removeTrack(char,track)
        ...
        if not hasActiveRecognizedAnimation(char) then
            clearAimingLine(char)
        else
            if not aimingLines[char] then
                createAimingLine(char)
            end
            if hasActiveYellowAnimation(char) then
                if not yellowAimingLines[char] then
                    createYellowAimingLine(char)
                end
            else
                clearLine(yellowAimingLines,char)
            end
    ...
    local function registerTrack(char,track)
        if not STATE.killerAimingEnabled and not lineAlways then
            return
        end
        ...
        -- Line of Sight has its own visual layer. Always Line only uses
        -- activeTracks to decide whether its yellow overlay is needed.
    ...
    local function bindCharacter(char)
        ...
        local conn=animator.AnimationPlayed:Connect(function(track)
            if STATE.killerAimingEnabled or lineAlways then
                registerTrack(char,track)
            end
        end)
    ...
    local function setEnabled(v)
        ...
        if not v then
            ...
            for char in pairs(yellowAimingLines) do
                clearAimingLine(char)
            end
            ...
            if not lineAlways then
                ...
                for char in pairs(alwaysYellowLines) do
                    clearAlwaysLine(char)
                end
            end
            return
    ...
    local function setAlwaysLineLength(value)
        alwaysLineLength=math.clamp(tonumber(value) or 50,10,100)
