...
do
    ...
    -- Pulse layer: a very early attack-state detector. It does NOT parry by
                ...
                if killerRoot then
                    ...
                    if pulseReady then
                        ...
                        if now-last>=PULSE_MIN_INTERVAL then
                            ...
                            -- Pulse is only an early trigger. The existing
                            -- canNormalAttackParry/doParry pipeline remains the
                            -- authoritative distance, facing, edge, pallet and
                            -- obstruction check.
                            if canNormalAttackParry(
                                killerChar,
                                distance,
                                ...
                                if trigger and doParry(true,killerChar,distance) then
                                    data.consumed=true
                                end
                            end
                        end
    ...
    RunService.RenderStepped:Connect(function()
        if STATE.autoParryEnabled then
            -- Extra render-frame pulse: the early pulse runs first, followed by
            -- the normal detector for a second independent check.
            normalAttackPulseMonitor()
            normalAttackersMonitor()
...
local KillerAimingFeature=(function()
    ...
    local HEAVEN_ARC_SEGMENTS=36
    local HEAVEN_ARC_DEGREES=120
    ...
    local function registerTrack(char,track)
        ...
        -- Always keep the white animation line as the base.
        if not aimingLines[char] then
            createAimingLine(char)
        end

        -- Lunge/Corrupt gets a separate yellow overlay instead of replacing
        -- the white line.
        if hasActiveYellowAnimation(char) then
            if not yellowAimingLines[char] then
                createYellowAimingLine(char)
