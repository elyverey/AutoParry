...
local KillerAimingFeature=(function()
    ...
    local function makeDashedTransparency()
        local keypoints={}
        local cycles=12
        local whitePortion=0.58
        local epsilon=0.001
        for i=0,cycles-1 do
            local base=i/cycles
            local nextBase=(i+1)/cycles
            local whiteEnd=base+(whitePortion/cycles)
            local gapStart=whiteEnd+0.002
            local gapEnd=nextBase-epsilon
            if i==cycles-1 then
                gapEnd=1
            end
            ...
            keypoints[#keypoints+1]=NumberSequenceKeypoint.new(base,0)
            keypoints[#keypoints+1]=NumberSequenceKeypoint.new(whiteEnd,0)
            if gapStart<gapEnd then
                keypoints[#keypoints+1]=NumberSequenceKeypoint.new(gapStart,1)
                keypoints[#keypoints+1]=NumberSequenceKeypoint.new(gapEnd,1)
            end
        end
        return NumberSequence.new(keypoints)
    end

    local DASHED_TRANSPARENCY=makeDashedTransparency()
    ...
    local function createAimingLine(char,color)
        ...
        beam.Transparency=DASHED_TRANSPARENCY
